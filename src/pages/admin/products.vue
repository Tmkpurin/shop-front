...
const submit = handleSubmit(async (vlaues) => {
  ...
  try {
    ...
    // fd.append(key, value)
    fd.append('name', vlaues.name)
    fd.append('price', vlaues.price)
    fd.append('description', vlaues.description)
    fd.append('category', vlaues.category)
    fd.append('sell', vlaues.sell)
    ...
    console.log(error)
    createSnackbar({
      text: t('api.' + (error?.response?.data?.message || 'unknownErorr')),
      snackbarProps: {
        color: 'red',
      },
    })
